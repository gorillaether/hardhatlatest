...
contract EchoVaultV3 {
    ...
    mapping(address => bool) public admins; // NEW: Admin management
    ...
    event UserTierUpdated(address indexed user, SubscriptionTier tier); // NEW
    ...
    // UPDATED: Now uses onlyAdmin instead of onlyOwner
    function updateUserTier(address _user, SubscriptionTier _tier) external onlyAdmin {
        userTiers[_user] = _tier;
        emit UserTierUpdated(_user, _tier);
    }
